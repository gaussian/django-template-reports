...
# For example, let’s assume the extra context comes from a Team model.
...
class ReportGenerationForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        choices = [('', '---------')] + [(opt.pk, str(opt)) for opt in available_context_options]
...
class ReportGenerationAdminMixin:
    actions = ['generate_reports']

    def generate_reports(self, request, queryset):
        selected = queryset.values_list('pk', flat=True)
        return HttpResponseRedirect(f"generate_reports/?ids={','.join(map(str, selected))}")
    ...
    def get_urls(self):
        urls = super().get_urls()
        custom_urls = [
            path('generate_reports/', self.admin_site.admin_view(self.generate_reports_view), name='generate_reports'),
        ]
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                if not disable_context1:
                    ...
                    if context1_id:
                        additional_context['context1'] = available_context_options.get(pk=context1_id)
                context2_id = form.cleaned_data.get('context2')
                if context2_id:
                    additional_context['context2'] = available_context_options.get(pk=context2_id)
                context3_id = form.cleaned_data.get('context3')
                if context3_id:
                    additional_context['context3'] = available_context_options.get(pk=context3_id)
                ...
                for record in queryset:
                    ...
                    rendered_file = render_pptx(report_def.pptx_template.path, context, output_path)
                    
                    # Capture the run data in JSON (for example, store context info).
                    run_data = {"context": {key: str(val) for key, val in context.items()}}
